...
12. [Дизайн и UI-система](#12-дизайн-и-ui-система)
...
## 12. Дизайн и UI-система

Приложение выполнено в стиле **Microsoft Fluent Design** — минималистичная светлая тема с акцентным синим цветом, характерная для современных продуктов Windows.
...
|------------|-----|-----------------|
| **Акцентный синий** | `#0078D4` | Шапка окна, активная вкладка, кнопки CTA, подчёркивание |
| **Тёмный синий** | `#005A9E` | Бейдж роли в шапке, hover-состояния заголовка |
| **Тёмный текст** | `#1F1F1F` / `#323130` | Заголовки, основной текст |
| **Вторичный текст** | `#605E5C` | Подписи, метки, даты |
| **Третичный текст** | `#8A8886` | Placeholder, неактивные элементы |
| **Фон страниц** | `#FAF9F8` | Фон всех Page (тёплый белый) |
| **Фон карточек** | `#FFFFFF` | Панели, карточки, таблицы |
| **Разделители** | `#E0E0E0` / `#F3F2F1` | Границы секций, строки таблиц |
| **Hover строки** | `#F8F8F8` | Лёгкая подсветка при наведении |
| **Выбранная строка** | `#EFF6FC` | Выделение в списках |
| **Зелёный (оценка 5)** | `#107C10` | Цвет полоски KPI-карточки, ячейки оценок |
| **Оранжевый (оценка 3)** | `#CA5010` | Предупреждения, тройки |
| **Красный (оценка 2)** | `#D13438` / `#E81123` | Двойки, кнопка закрытия окна |
| **Фиолетовый** | `#8764B8` | KPI-карточка «Преподаватели» |
| **Жёлтый (темы уроков)** | `#FFFBE6` | Строка «📝 Тема урока» в журнале |

---

### 12.2 Типографика

Всё приложение использует единственный шрифт — **Segoe UI** (системный шрифт Windows).

| Контекст | Размер | Насыщенность |
|----------|--------|-------------|
| Логотип («EduTrack Pro») | 50px | Bold, Arial Black |
| Заголовки страниц | 18px | SemiBold |
| Заголовки секций | 13px | SemiBold |
| KPI-цифры на дашборде | 26px | Bold |
| Основной текст / таблицы | 12px | Regular |
| Мелкие подписи, метки | 10–11px | Regular |
| Вкладки навигации | 12px | Regular → SemiBold (активная) |

---

### 12.3 Структура главного окна

```
┌─────────────────────────────────────────────────────────────────┐
│  ШАПКА (49px) — #0078D4                                         │
│  [Лого] [EduTrack Pro]    [Группа]    [Роль][Имя][🔔][Выйти][─□✕] │
├─────────────────────────────────────────────────────────────────┤
│  НАВИГАЦИЯ (31px) — белая полоса с нижней границей #E0E0E0      │
│  Главная | Студенты | Посещаемость | Успеваемость | Расписание … │
│          ───── (активная вкладка — синяя полоска снизу)          │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  КОНТЕНТ (#FAF9F8)                                               │
│  Frame → Page (каждая вкладка — отдельная Page)                  │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```

**Шапка** — кастомное окно без системного `WindowStyle` (`WindowStyle="None"` + `WindowChrome`). Кнопки сворачивания/разворачивания/закрытия реализованы вручную; при наведении на «✕» фон становится `#E81123` (красный).

**Навигация** — горизонтальная полоса кнопок-вкладок. Активная вкладка выделяется синей полоской (`BorderThickness="0,0,0,3"`, `BorderBrush="#0078D4"`), текст становится синим и SemiBold. При наведении — фон `#F3F3F3`.

---

### 12.4 Компонент: KPI-карточка (Дашборд)

```
┌──────────────────────────┐
│████████████████████████  │  ← цветная полоска 4px (цвет зависит от метрики)
│                           │
│  26                       │  ← крупная цифра, Bold, #1F1F1F
│  Студентов                │  ← подпись, 11px, #605E5C
└──────────────────────────┘
```

8 карточек в 2 ряда по 4 (только для Admin). Для других ролей — 4 карточки в 1 ряд. Все карточки белые с границей `#E0E0E0`.

---

### 12.5 Компонент: журнал оценок (DataGrid)

- **Строки** — данные студентов; ячейки с оценками получают цветной фон через `IValueConverter`
- **Строка «📝 Тема урока»** — жёлтый фон `#FFFBE6`, первая в таблице (`StudentId = -1`)
- **Ячейки оценок** — скруглённые Border внутри DataGridTemplateColumn; цвет зависит от значения:

| Оценка | Фон ячейки | Цвет текста |
|--------|-----------|-------------|
| 5 | `#107C10` (зелёный) | Белый |
| 4 | `#0078D4` (синий) | Белый |
| 3 | `#CA5010` (оранжевый) | Белый |
| 2 | `#D13438` (красный) | Белый |
| Тема урока | Прозрачный | Синий `#0078D4` |
| Пусто | Прозрачный | — |

- **Заголовки дней с темой** — синий цвет + подчёркивание + Tooltip с полным текстом темы
- **Popup ввода оценки** — всплывающее окно с DropDown (2–5), кнопками «Сохранить» и «Удалить»
- **Popup темы урока** — TextBox с максимумом 300 символов, кнопки «Сохранить» и «Очистить»

---

### 12.6 Компонент: прогресс-бар посещаемости

Используется на дашборде Admin (блок «Посещаемость сегодня»). Реализован без `ProgressBar`-контрола — через наложение двух `Rectangle` внутри `Grid`:

```xml
<!-- Серый фон -->
<Rectangle Height="6" Fill="#E0E0E0" RadiusX="3" RadiusY="3"/>
<!-- Синяя полоса — Width вычисляется: присутствует / всего × 200 -->
<Rectangle Height="6" Fill="#0078D4" HorizontalAlignment="Left"
           Width="{Binding BarWidth}" RadiusX="3" RadiusY="3"/>
```

---

### 12.7 Компонент: уведомления (Popup)

Колокольчик 🔔 в шапке открывает `Popup` шириной 370px с тенью (`DropShadowEffect`). Непрочитанные объявления отмечаются синей точкой и чуть голубым фоном (`#12 0078D4`). При открытии все уведомления автоматически помечаются прочитанными; ID последнего просмотренного сохраняется в `%LOCALAPPDATA%\CollegeJournalApp\notif_{userId}.dat`.

---

### 12.8 Адаптация под роли

Интерфейс не имеет фиксированного набора элементов — он собирается динамически при входе:

- **Вкладки** скрываются/показываются через `Visibility` в `Window_Loaded` на основе `SessionHelper.RoleName`
- **Дашборд** показывает разные панели (`AdminStatsPanel`, `AdminDashboardBody`, `NonAdminContent`) в зависимости от роли
- **Журнал оценок** — кнопки «Сохранить» и попап редактирования доступны только если пользователь является преподавателем этого предмета (`_editableSubjectIds`)
- **Карточка студента** — вкладки «Социальный статус» и «Документы» скрыты для ролей Headman и Student
